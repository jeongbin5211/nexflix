...
const Row: React.FC<RowProps> = ({ isLargeRow, title, id, fetchUrl}) => {
  ...
  const handleClick = (movie: Movie) => {
    setModalOpen(true); // 클릭시 모달이 열리도록 설정
  ...
  return (
    <section className='row'>
      ...
      <div id={id} className='row__posters' style={{ display: 'flex', overflowX: 'scroll', scrollSnapType: 'x manatory'}}>
        {movies.map((movie) => (
      ...
      {/* modalOpen이 true일 때, movieSelected에 영화 정보가 있을 때 MovieModal 렌더링 */}
